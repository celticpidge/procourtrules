...
export default function ChatWindow({ messages, isLoading, error, remaining, onSend }) {
  ...
  function startVoiceSession(mode) {
    ...
    trackTelemetry('voice_start', {
      mode,
      forced_progressive: forceProgressivePreviewRef.current,
    });
  }

  function trackFirstText(source) {
    if (firstTextLoggedRef.current || !voiceSessionStartAtRef.current) {
      return;
    }
  ...
  function beginAutoStopWatchers(stream, recorder) {
    ...
    // Hard stop to avoid very long captures if silence detection is unavailable.
    autoStopTimeoutRef.current = setTimeout(() => {
      if (recorder.state === 'recording') {
        autoStopReasonRef.current = 'max_duration';
        recorder.stop();
      }
    }, MAX_RECORDING_MS);
    ...
    try {
      ...
      silenceIntervalRef.current = setInterval(() => {
        ...
        if (silenceMsRef.current >= SILENCE_MS_TO_STOP) {
          autoStopReasonRef.current = 'silence';
          recorder.stop();
        }
      }, CHECK_INTERVAL_MS);
    } catch {
      // If the browser blocks audio analysis, max-duration stop still protects UX.
    }
  }

  async function updateProgressiveTranscript() {
    // If browser speech preview is actively producing results, avoid extra network calls.
    if (speechSupported && livePreviewHasResultsRef.current) {
      return;
    }

    if (previewTranscriptionInFlightRef.current) {
      return;
    }

    const now = Date.now();
    if (now - previewTranscriptionLastAtRef.current < 1400) {
      return;
    }
    ...
    try {
      ...
      if (!progressiveErrorLoggedRef.current) {
        ...
        trackTelemetry('voice_transcribe_error', {
          mode: sessionModeRef.current,
          stage: 'progressive',
          code: 'progressive_failed',
        });
      }
      // Ignore progressive failures; final transcription on stop is authoritative.
    } finally {
  ...
  function startLivePreviewRecognition() {
    if (!speechSupported || forceProgressivePreviewRef.current) {
      return;
    }
    ...
    try {
      ...
      recognition.onresult = (event) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i += 1) {
          ...
          if (event.results[i].isFinal) {
            livePreviewFinalTranscriptRef.current += `${text} `;
          } else {
            interimTranscript += text;
          }
        }

        const spokenText = `${livePreviewFinalTranscriptRef.current}${interimTranscript}`.trim();
        const baseText = inputBeforeVoiceRef.current;
        const nextValue = spokenText
          ? (baseText ? `${baseText} ${spokenText}` : spokenText)
          : baseText;
        setInput(nextValue);
      ...
      recognition.onerror = () => {
        ...
        trackTelemetry('voice_live_preview_failed', {
          mode: sessionModeRef.current,
          reason: 'speech_error',
        });
  ...
  async function startRecorderTranscription() {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        trackTelemetry('voice_auto_stop', {
          mode: sessionModeRef.current,
          reason: stopReason,
        });
        ...
        try {
          ...
          trackTelemetry('voice_transcribe_error', {
            mode: sessionModeRef.current,
            stage: 'final',
            code: classifyTranscriptionError(err?.message || ''),
          });
  ...
  function startSpeechRecognitionFallback() {
    ...
    recognition.onstart = () => {
      startVoiceSession('speech_fallback');
      setIsVoiceListening(true);
      setIsVoiceRecordingFallback(false);
    };
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
        setVoiceError('Microphone access was denied by the browser or OS. Check site and system microphone settings.');
        return;
      }
      ...
      if (event.error === 'network') {
        if (!navigator.onLine) {
          setVoiceError('You appear to be offline. Reconnect to the internet and try voice input again.');
          return;
        }
        if (isElectronBrowser) {
          setVoiceError('Speech recognition network service is unavailable in the VS Code embedded browser. Open this app in Chrome or Edge and try again.');
          return;
        }
